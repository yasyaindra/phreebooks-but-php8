...
class SMTP
{
    ...
    protected function sendCommand($command, $commandstring, $expect)
    {
        ...
        if ($this->do_debug >= 2) {
        	$this->edebug('send command CLIENT -> SERVER: ' . strip_tags($commandstring));
        }
        ...
        if ($this->do_debug >= 2) {
            $this->edebug('send command SERVER -> CLIENT: ' . strip_tags($reply));
        }
        ...
        if (!in_array($code, (array)$expect)) {
            ...
            if ($this->do_debug >= 1) {
                $this->edebug(
                    "SMTP ERROR CODE: $code ERROR: {$this->error['error']}: $reply"
                );
            }
            return false;
        }
