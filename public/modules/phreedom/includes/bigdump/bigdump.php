...
// Staggered import of an large mysqli Dump (like phpMyAdmin 2.x Dump)
...
// *** Add mysqli module availability check
// *** Workaround for mysqli_close() bug #48754 in PHP 5.3
...
$comment[]='-- ';
// $comment[]='---';                  // Uncomment this line if using proprietary dump created by outdated mysqlidump
...
// See http://dev.mysqli.com/doc/refman/5.0/en/charset-charsets.html for the full list
...
<!-- <h1>BigDump: Staggered mysqli Dump Importer ver. <?php echo (VERSION); ?></h1> -->
...
// ****************************** BOF - Mods by PhreeSoft ****************************************
echo BIGDUMP_INTRO;
//echo ('<h1>BigDump: Staggered mysqli Dump Importer v'.VERSION.'</h1>');
// ****************************** EOF - Mods by PhreeSoft ****************************************
skin_close();
...
{ $dbconnection = @mysqli_connect($db_server,$db_username,$db_password);
  ...
  if (!$error && $db_connection_charset!=='')
    @mysqli_query("SET NAMES $db_connection_charset", $dbconnection);
}
else
{ $dbconnection = false;
}
...
// Print the current mysqli connection charset

if (!$error && !TESTMODE && !isset($_REQUEST["fn"]))
{ 
  $result = mysqli_query("SHOW VARIABLES LIKE 'character_set_connection';");
  $row = mysqli_fetch_assoc($result);
  if ($row) 
  { $charset = $row['Value'];
    echo ("<p>Note: The current mysqli connection charset is <i>$charset</i>. Your dump file must be encoded in <i>$charset</i> in order to avoid problems with non-latin characters. You can change the connection charset using the \$db_connection_charset variable in bigdump.php</p>\n");
  }
}
