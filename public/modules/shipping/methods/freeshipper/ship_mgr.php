...
// +-----------------------------------------------------------------+
//  Path: /modules/shipping/methods/freeshipper/ship_mgr.php
//
?>
...
<table class="ui-widget" style="border-collapse:collapse;width:100%">
 <thead class="ui-widget-header">
  <tr>
    <th colspan="8"><?php echo SHIPPING_FREESHIPPER_SHIPMENTS_ON . gen_locale_date($date); ?></th>
  </tr>
  <tr>
  ...
  <?php 
	...
	if ($result->RecordCount() > 0) {
		while(!$result->EOF) {
			...
			echo '    <td align="center">' . constant('freeshipper_' . $result->fields['method']) . '</td>' . chr(10);
			echo '    <td align="right">' . ($result->fields['deliver_date'] ? gen_locale_date($result->fields['deliver_date']) : '&nbsp;') . '</td>' . chr(10);
...
//	  		echo html_icon('actions/document-print.png', TEXT_PRINT, 'small', 'onclick="window.open(\'' . html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('method', 'labels', 'date')) . 'method=freeshipper&date=' . $date . '&labels=' . $result->fields['tracking_id'], 'SSL') . '\',\'label_mgr\',\'width=800,height=700,resizable=1,scrollbars=1,top=50,left=50\')"') . chr(10);
	  		echo html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SHIPPING_DELETE_CONFIRM . '\')) window.open(\'index.php?module=shipping&page=popup_label_mgr&method=freeshipper&sID=' . $result->fields['shipment_id'] . '&action=delete\',\'popup_label_mgr\',\'width=800,height=700,resizable=1,scrollbars=1,top=50,left=50\')"') . chr(10);
			echo '    </td>';
			echo '  </tr>' . chr(10);
			$result->MoveNext();
		}
